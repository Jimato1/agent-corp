...
│  │  L2 · Gateway physical stop  CONFIRMED   source: Gateway direct · 0.4s ago   │ │
...
1. **L1-applied vs L2-confirmed — with L2 freshness discipline** (PLAN §7.1, §1.2; **amended 2026-07-02 per merge H5 / `killswitch-chain.md` §4**). Two independent rows, because either layer alone stops action and the operator must see *both* landed. L1 can be `APPLIED` while L2 is still `PENDING`; never collapse into one check. **The Gateway's own confirmation, read DIRECTLY by auth, is the SOLE legitimate source of L2-CONFIRMED** — the kill bites physically at the Gateway; an MC relay is never an L2-CONFIRMED source. The L2 row carries the **same freshness discipline as the per-RS table**: a mono **as-of / read-age stamp** (`0.4s ago`), an explicit **`source: Gateway direct`** label, and three defined states — **`PENDING`** (no direct ack yet), **`CONFIRMED`** (fresh direct Gateway ack within bound), and **`STALE / UNKNOWN`** (read age past bound, or only relayed/mirrored data available → *"cannot confirm L2 from here — verify at Mission Control ▸"*, **never silently "CONFIRMED"**). A stale or relayed value must never read as a live physical-stop confirmation. The "either layer alone halts action" copy stays.
...
- **C3 · medium · HONESTY/SAFETY BREAK — Halt Status Board showed L2 Gateway physical-stop as flat "CONFIRMED" with no freshness/source qualifier, though auth holds only a read-only mirror.** **RESOLVED.** §5.2 gives the L2 row the **same freshness discipline as the per-RS table**: a mono **read-age stamp** (`0.4s ago`), an explicit source label, and defined states **PENDING / CONFIRMED / STALE-UNKNOWN** (`cannot confirm L2 from here — verify at Mission Control ▸`). A stale value never reads as a live confirmation; the "either layer alone halts action" copy stays. Mirrored into BG-3 (§6.8). *(Amended 2026-07-02, merge H5/R9: the source label is `source: Gateway direct` — auth reads the Gateway itself; a "Gateway/MC mirror" is no longer an admissible L2-CONFIRMED source.)*
