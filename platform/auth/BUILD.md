...
| CV-4 | **Redis pub/sub cross-replica fan-out** (denylist propagation, sub-second revocation, read-your-writes-under-lag, introspect-fail-closed-on-Redis-loss) | no `redis-server` | `docker run -p 6379:6379 redis:7 --appendonly yes && cd platform/auth && python -m pytest tests/integration/test_redis_fanout.py` |
