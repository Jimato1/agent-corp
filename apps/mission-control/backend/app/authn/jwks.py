...
MC is an OAuth2 resource server (`aud == mc`): it validates auth's `at+jwt` tokens
LOCALLY against auth's published JWKS (no third-party JWT library — the house value
from ``platform/auth``: hand-rolled over ``cryptography``). Supported production algs:
...
``MC_AUTH_TEST_HS256_SECRET`` is configured.
...
every token signed by it within one poll interval (<=30s). This is the ONLY auth
kill channel MC depends on client-side — MC never becomes a second enforcement point.
"""
