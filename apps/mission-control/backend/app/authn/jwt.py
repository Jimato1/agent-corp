...
Local, offline RS validation per auth PLAN §4.7 / §8 claim shape. Resolve the key
(is the ``kid`` current?) and verify the signature BEFORE trusting any claim value.
No third-party JWT library — hand-rolled compact-JWS over the ``cryptography``-backed
verifiers in :mod:`app.authn.jwks`, matching the house pattern in ``platform/auth``.
"""
...
def validate_access_token(
    ...
    *,
    ...
    """Validate an ``at+jwt`` and return its claims dict.

    Enforces: signature, ``kid`` current, ``alg`` match, ``typ==at+jwt``,
    ``iss==expected_iss`` (RFC 9207), ``aud==expected_aud`` single-valued (RFC 8707),
    and ``exp`` not passed (with skew). Revocation is a separate live concern; the
    benign fast path accepts <= TTL staleness (agent TTL is 2 min, contract §2).
    """
...
def decode_identity_header(
    ...
    *,
    ...
    """Verify the proxy-injected ``X-Auth-Identity`` JWS and return its claims.

    The proxy sets this on 200 (auth PLAN §8.7); a backend MUST cryptographically
    verify signature + ``aud`` before trusting any claim. It is signed with a key
    DISTINCT from the access-token key (separate ``kid``); we resolve it through the
    same key ring by ``kid`` (auth publishes the identity key with its own ``kid``).

    NOTE: the exact ``typ`` string of the identity header is auth-internal; we do not
    pin it (we verify signature + iss + aud + exp), so a benign ``typ`` change on
    auth's side never breaks this seam.
    """
    h_seg, p_seg, s_seg = _split(token)
