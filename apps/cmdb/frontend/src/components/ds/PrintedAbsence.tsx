...
/* Helm — PrintedAbsence
   Where a capability CANNOT exist by construction, render it as a CALM PRINTED
   FACT with a 🔒/⛊ lock glyph and NO control at all — never a greyed-out toggle
   (a disabled toggle implies the power exists and could be switched on). */
...
injectStyle('helm-printedabsence-css', CSS);

export interface PrintedAbsenceProps extends HTMLAttributes<HTMLDivElement> {
  glyph?: ReactNode;
  children: ReactNode;
  why?: ReactNode;
  tag?: ReactNode;
...
export function PrintedAbsence({ glyph = '🔒', children, why, tag = 'by construction', className = '', ...rest }: PrintedAbsenceProps) {
  return (
