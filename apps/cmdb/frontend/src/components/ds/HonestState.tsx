...
/* Helm — HonestState (the honest-stop triad)
   ...
   always visible even at zero: ✔ confirmed · ◐ pending · ⇉ draining.
   Copy discipline is ABSOLUTE: never say "all stopped" while pending or
   draining is above zero. */
...
injectStyle('helm-honeststate-css', CSS);

export interface HonestStateProps extends HTMLAttributes<HTMLDivElement> {
  confirmed?: number;
  pending?: number;
  draining?: number;
...
export function HonestState({
  ...
  return (
    <div className={cls} {...rest}>
      ...
      {draining > 0 && drainingDetail ? <div className="helm-honest__detail">draining: {drainingDetail}</div> : null}
      {summary
        ? settled
