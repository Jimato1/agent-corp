...
Local, offline RS validation per auth PLAN §5.1 / §8 claim shape. Resolve the key (is
the ``kid`` current?) and verify the signature BEFORE trusting any claim value. No
third-party JWT library — hand-rolled compact-JWS over the ``cryptography``-backed
verifiers in :mod:`app.authn.jwks`.

``validate_access_token`` returns BOTH the verified JOSE header and the claims, because
the §8 holder-token validation in :mod:`app.authn.principal` needs the header ``kid`` for
the live denylist check (which keys ``jti`` + ``sub`` + ``kid`` + kill level).
"""
...
def validate_access_token(
    ...
    *,
    ...
    ``exp`` not passed (with skew). Revocation / holder extras are handled by the caller.
    """
...
def decode_identity_header(
    ...
    *,
    ...
    The proxy sets this on 200 (auth PLAN §8.7); a backend MUST cryptographically verify
    signature + ``aud`` before trusting any claim. It is signed with a key DISTINCT from
    the access-token key (separate ``kid``); resolved through the same ring by ``kid``.
    """
    h_seg, p_seg, s_seg = _split(token)
