...
* 401 — missing/invalid credential → re-mint (``WWW-Authenticate: Bearer`` + RFC 9728
  metadata pointer).
* 403 — authenticated but insufficient scope / PDP-deny / refused at the door.
* 409 — idempotency / propose-confirm drift conflict.
* 429 — budget / rate limit.
* 503 — a fail-closed dependency outage.

Business outcomes on the MCP surface do NOT use these — they return a structured
``isError`` tool result (Board convention). A policy ``deny`` is NOT an error: it is a
valid verdict returned 200 on the decision endpoint.
"""
