...
/* Helm — ConfirmFriction (the step-up confirm ceremony)
   The single gate for every dangerous op suite-wide. Two intensities:
...
injectStyle('helm-confirm-css', CSS);
...
export function ConfirmFriction({
  ...
  const onKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    if (e.key === 'Escape') {
      e.preventDefault();
      if (e.shiftKey) onEscapeToHalt?.();
      else onCancel?.();
    }
    if (e.key === 'Tab' && dialogRef.current) {
      const f = dialogRef.current.querySelectorAll<HTMLElement>('button, input, a[href], [tabindex]:not([tabindex="-1"])');
      if (!f.length) return;
      const first = f[0];
        ...
        <div className="helm-confirm__body">
          <div className="helm-confirm__consequence">
            ...
            <div className="helm-confirm__meta">
              ...
              {blastRadius ? (
                <span className="helm-confirm__tag helm-confirm__tag--less" style={{ background: 'var(--bg-control)', color: 'var(--text-secondary)', borderColor: 'var(--border-strong)' }}>
          ...
          {auditNote ? <div className="helm-confirm__audit"><span aria-hidden="true">⛓</span> {auditNote}</div> : null}
        </div>

        <div className="helm-confirm__foot">
          <button ref={cancelRef} type="button" className="helm-cf-btn helm-cf-btn--cancel" onClick={() => onCancel?.()}>Cancel</button>
          <button
            type="button"
...
export default ConfirmFriction;
