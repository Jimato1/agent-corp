...
The Gateway is a resource server: it validates auth's ``at+jwt`` tokens LOCALLY against
auth's published JWKS (no third-party JWT library — the house value: hand-rolled over
``cryptography``). Supported production algs: ``EdDSA`` (Ed25519) and ``ES256`` (P-256).
``HS256`` is reachable ONLY when ``GATEWAY_AUTH_TEST_HS256_SECRET`` is configured
(isolated-build test signer); a real JWKS never publishes symmetric keys.
...
token signed by it within one poll interval (<=30s). Refresh REPLACES the JWKS-sourced set
wholesale so a retired kid never lingers.

Identical to apps/cmdb's KeyRing (the house RS baseline) — the Gateway is one more RS.
"""
