...
injectStyle('helm-principalref-css', CSS);

export type PrincipalKind = 'agent' | 'operator' | 'service';

export interface PrincipalRefProps extends HTMLAttributes<HTMLSpanElement> {
...
const KIND_GLYPH: Record<PrincipalKind, string> = { agent: '⬡', operator: '◐', service: '⚙' };

export function PrincipalRef({ id, kind = 'agent', href, status = 'active', className = '', ...rest }: PrincipalRefProps) {
  ...
  return (
    <span className={['helm-principal', className].filter(Boolean).join(' ')} {...rest}>
      {href ? <a className={chipCls} href={href}>{inner}</a> : <span className={chipCls}>{inner}</span>}
      {status === 'revoked' ? (
