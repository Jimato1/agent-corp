...
- **Board:** design the approval record (`approval_id`, `plan_hash`, per-approval allowlist) + `consume_approval` API + PIP facts endpoint (proposer_id, ticket state) for auth's PDP; record proposer/approver and enforce four-eyes; adopt T- id format + full state superset; outage-aware reaper; `team` label; WIP-cap write surface for MC; ARCH §12 lane-eligibility computation honoring provenance taint (incl. Library's curation-agent taint); backup mechanism for its CANONICAL DB; host svc:tier-approver (if D-15 ratified); spawn-depth caps (if D-11 ratified).
...
- **CMDB:** task-type registry ownership incl. external-verifier binding; one canonical verdict shape (already frozen); host-facts additions (D-6); `disposable` class (D-7); host onboarding lifecycle (gap 7.3); fix `/_context/` path refs in CLAUDE.md.
- **Vault:** blocked on auth pinning the holder-scope claim shape — sequence auth's countersign FIRST; then Stage-2 per the frozen contract; unsealer + WORM sink deployment amendments (D-16); fix `/_context/` path refs.
