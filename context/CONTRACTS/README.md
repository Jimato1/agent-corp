...
| auth → every app: RS baseline + scope→tool countersign index | `auth-apps-tokens-scopes.md` | FROZEN by reference to auth PLAN; per-app countersigns tracked there (session #2 2026-07-06: `svc:vault`/`svc:cmdb`/`svc:library` registered §9; key-provisioning verdict §12; Redis-S5 resolution §11.1) |
| agent-runtime → auth: key-provisioning (C7) + token-binding (C8) | `agent-runtime-auth-key-provisioning.md` | **FROZEN + COUNTERSIGNED** (runtime 2026-07-04; auth 2026-07-06 — verdict `auth-apps-tokens-scopes.md` §12). `attestation` byte-format finalizes at G1 |
...
- ~~agent-runtime ↔ auth: key-provisioning (C7/C8)~~ — **DONE: frozen `agent-runtime-auth-key-provisioning.md` (runtime side 2026-07-04) and COUNTERSIGNED by auth 2026-07-06** (auth session #2; verdict `auth-apps-tokens-scopes.md` §12). Enrollment payload, TPM2_Certify attestation, EK-allow-list ownership (auth's), DPoP-mint + rotation/revocation all reconciled to auth PLAN §3.6/§4.5/§8; `attestation` byte-format finalizes at the G1 hardware spike. See the frozen-table row above
