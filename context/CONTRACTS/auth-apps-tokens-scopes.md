...
| cmdb | **COUNTERSIGNED at CMDB Stage-2 (2026-07-03, `apps/cmdb/planning/PLAN.md` §7.1):** `cmdb:read-policy` (all read tools + the binding `POST /v1/decision` + Library host-facts — the ONE canonical read scope; the hostfacts contract's "`cmdb:read`" is an erratum naming this scope) / `cmdb:write-policy` (HOLDER, human-only per the §8 pin — every policy/registry/bind/sandbox/break-glass mutation; sod-critical) / **`cmdb:manage` proposed** (non-holder, human-only, write-benign: sync-trigger, drift-ack — mechanical §7-note addition). Grants consumed/asked: `svc:tier-approver` → `cmdb:read-policy` (§9, already registered); `svc:gateway` → `cmdb:read-policy` (raised by Gateway Stage-2); `svc:cmdb` registration asked (Board escalation outbox — CMDB PLAN §11-A2) |
