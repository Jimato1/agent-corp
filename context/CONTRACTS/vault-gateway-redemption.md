...
## 3. Where the Board+CMDB preconditions are checked — FLAGGED (operator ratifies, D-4)

Both sides agree plaintext is released only for an approved ticket in-window; they disagree on **where** that is verified (Vault research is internally split: §7 "Vault re-verifies itself" vs §5/Rec4 "upstream at the Gateway; Vault is the fourth lock"). **Recommendation (not yet binding):** baseline = Gateway-side enforcement (the Gateway is the PEP and already gathers Board approval + CMDB verdict), **plus** Vault's redeem endpoint independently verifying the approval evidence — naming **`approval_id`** explicitly (both researches omitted it; IDENTIFIERS.md posture "validated live" applies) — either by one Board API call at release time or by verifying a Board-signed approval evidence offline. RFC 9396 RAR (`authorization_details` naming path + ticket) is the stronger auth-side lever if auth supports it. Whichever side is ratified, the standing regression is unchanged: **agent token → redeem → 403, always**, and a denied agent redemption is a first-class exfiltration signal (logged + escalated).
