...
export function HaltBand({
  ...
  const glyph = kill ? (g2 ? '▮▮▮▮' : '▮▮') : '⛊';
  const word = kill
    ? (g2 ? 'FULL QUIESCE — ALL AGENTS HALTED' : 'KILL-SWITCH ENGAGED')
    : 'SYSTEM SAFE-STOPPED';
  ...
  return (
    <div className={cls} role="status" {...rest}>
      ...
      <div className="helm-halt__aside">
        {showTriad
          ? <HonestState
              confirmed={confirmed} pending={pending} draining={draining}
              pendingCountdown={pendingCountdown} drainingDetail={drainingDetail}
              summary={false} />
          : null}
