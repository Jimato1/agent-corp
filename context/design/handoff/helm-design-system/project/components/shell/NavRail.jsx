  ...
  return (
    <nav className={['helm-rail', className].filter(Boolean).join(' ')} data-collapsed={collapsed ? 'true' : 'false'} {...rest}>
      <div className="helm-rail__brand">
        ...
        <button className="helm-rail__toggle" aria-label={collapsed ? 'Expand rail' : 'Collapse rail'} onClick={() => onToggle && onToggle(!collapsed)}>
          {collapsed ? '»' : '«'}
        </button>
      </div>
...
function ItemLink({ item }) {
  ...
  if (item.href) return <a className={cls} href={item.href} title={item.label} aria-current={item.active ? 'page' : undefined}>{content}</a>;
  return <button className={cls} title={item.label} onClick={item.onClick} aria-current={item.active ? 'page' : undefined}>{content}</button>;
}
