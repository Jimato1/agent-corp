...
(function () {
  ...
  const { StatusPill, FreshnessStamp, PrincipalRef, TicketRef, FenceState, Button, HonestState } = H;
  ...
  const panelStyle = { background: 'var(--bg-card)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-panel)' };

  /* ⟳ age · source: X  — honest freshness stamp. When stale, degrades to gold. */
  function SourceStamp({ source, age, stale, staleLabel }) {
    if (stale) {
  ...
  function KillLevelPill({ level = 'G0', size }) {
  ...
  function Panel({ title, stamp, children, deepLabel, onDeep, pad = 16, style }) {
    return (
  ...
  function Liveness({ agent, stale }) {
    ...
    return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
        ...
        {agent.phi != null ? <span style={{ ...mono, fontSize: 12, color: agent.liveness === 'suspect' ? 'var(--state-amber-ink)' : 'var(--text-muted)' }}>φ{agent.phi}</span> : null}
        {agent.hb && agent.hb !== '—'
          ? <span style={{ ...mono, fontSize: 12, color: agent.liveness === 'suspect' ? 'var(--state-amber-ink)' : 'var(--text-muted)' }}>♥{agent.hb}</span>
  ...
  function AttentionBand({ agents, onOpen }) {
          ...
          {flagged.map((a) => {
            ...
            return (
              <div key={a.sub} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 8px', flexWrap: 'wrap' }}>
                ...
                {a.step != null ? <span style={{ ...mono, fontSize: 12, color: 'var(--text-muted)' }}>step {a.step}</span> : null}
                {a.ticket ? <TicketRef id={a.ticket} href="#" /> : null}
                {a.fence && a.fence.state === 'superseded'
                  ? <FenceState gen={a.fence.gen} supersededBy={a.fence.supBy} state="superseded" />
  ...
  function FleetAnomalyBanner({ suppressed = 0 }) {
    return (
  ...
  function SpawnTree({ nodes, depth, cap }) {
    return (
      <div style={{ ...mono, fontSize: 12, lineHeight: '22px', color: 'var(--text-secondary)' }}>
        {nodes.map((n, i) => (
          <div key={i} style={{ paddingLeft: n.indent * 16, color: n.here ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
        ...
        <div style={{ marginTop: 6, color: 'var(--text-muted)' }}>depth {depth} / cap {cap}</div>
      </div>
    );
  }

  /* BudgetMeter — one of the four dimensions (rate/concurrency/cooldown/lifetime). NEVER dollars. */
  function BudgetMeter({ label, pct, value, trip, width = 96 }) {
  ...
  function EdgeTile({ tile, stale }) {
    return (
      <div style={{ ...panelStyle, padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontFamily: 'var(--font-ui)', fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>{tile.app}</span>
          <SourceStamp source="mc_prometheus" age="4s" stale={stale} staleLabel="CANNOT CONFIRM" />
        </div>
        {stale ? (
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: 12, color: 'var(--halt-gold-ink)' }}>Cannot confirm edge health — sidecar unreachable.</div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 12px', ...mono, fontSize: 12, color: 'var(--text-secondary)' }}>
            <span>2xx <b style={{ color: 'var(--text-primary)' }}>{tile.ok}</b></span>
            <span>rps {tile.rps}</span>
            <span>p95 {tile.p95}</span>
            <span>cert {tile.cert}</span>
            <span style={{ gridColumn: '1 / -1' }}>authz {tile.authz}</span>
            <span style={{ gridColumn: '1 / -1' }}>scrub_stripped {tile.scrub}</span>
          </div>
        )}
      </div>
    );
  ...
  function HaltNotConfirmed({ onDismiss }) {
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'var(--halt-gold-wash)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32 }}>
        <div style={{ maxWidth: 620, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <a href="#" onClick={(e) => { e.preventDefault(); onDismiss && onDismiss(); }} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 40, padding: '0 20px', borderRadius: 'var(--radius-control)', background: 'var(--halt-gold)', color: '#2E1D0B', fontFamily: 'var(--font-ui)', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>Open auth's safe-stopped console →</a>
            <button onClick={onDismiss} style={{ height: 40, padding: '0 16px', borderRadius: 'var(--radius-control)', background: 'transparent', border: '1px solid var(--halt-gold-edge)', color: 'var(--halt-gold-ink)', fontFamily: 'var(--font-ui)', fontSize: 13, cursor: 'pointer' }}>Dismiss (demo)</button>
          </div>
        </div>
      </div>
    );
  ...
  window.MCParts = { SourceStamp, KillLevelPill, Panel, Liveness, AttentionBand, FleetAnomalyBanner, SpawnTree, BudgetMeter, EdgeTile, HaltNotConfirmed, eyebrow, mono, panelStyle };
})();
